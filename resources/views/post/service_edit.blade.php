    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            if (index != -1) {
                ...
                $( '#'+parentId.replace(/\./g, '\\.') ).addClass('isActiveBtn'); // add active effect on chosen sub tag
                index = parentId.lastIndexOf('.'); // find dot in chosen sub tag
                if ( index != -1 ) {
                    grandParentId = parentId.substr(0, index); // get chosen sub sub tag
                    $('div.tags_'+grandParentId.replace(/\./g, '\\.')).removeClass('hidden'); // show shosen sub sub tags column
                    $( '#'+grandParentId.replace(/\./g, '\\.') ).addClass('isActiveBtn'); // add active effect on chosen sub sub tag
                }
            }
