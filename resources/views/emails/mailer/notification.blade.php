...
<html lang="{{$lang}}">
    ...
    <body>
        <h1 style="margin-top:20px;text-align:center;"><a style="font-size:125%;" href="{{route('home')}}">{{env('APP_NAME')}}</a></h1>
        <div style="width:550px;height:auto;margin:20px auto;background-color:#efefef;padding:20px;">
            ...
            <p style="white-space:pre-line;margin-bottom:15px;">{{__('ui.mailerNotifBody', ['reason' => $reason])}}:
                ...
                {{__('ui.mailerNotifBody3') }}: <a href="{{ route('posts.show', $post->id) }}">{{ route('posts.show', $post->id) }}</a>.
            </p>
            <p id="slg">{{__('ui.mailerNotifSlg')}}<br>
            <a class="mailer-link" href="{{route('home')}}">{{ env('APP_NAME') }}</a></p>
        </div>
        <p style="text-align:center;margin-bottom:15px;">&copy; {{ env('COPY_RIGHT_YEAR') }} <a class="mailer-link" href="{{route('home')}}">{{ env('APP_NAME') }}</a>. {{__('ui.footerCopyright')}}</p>
    </body>
</html>